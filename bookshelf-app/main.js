...
const createReadButton = () => createButton("read-button", event => addBookToCompleted(event.target.parentElement));
...
const createTrashButton = () => createButton("trash-book", event => removeBookFromCompleted(event.target.parentElement));
...
const createUnreadButton = () => createButton("unread-button", event => undoBookFromCompleted(event.target.parentElement));
...
const searchBooks = () => {
  ...
  const filteredBooks = books.filter(book => book.bookTitle.toLowerCase().includes(searchInput));
